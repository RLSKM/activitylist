...
def home():
    ...
    if tag and tag != '0':  # '0' represents the default 'No Filter'
        cursor.execute("""
        SELECT activity.name, activity.description
        FROM Activity
        INNER JOIN activity_genre
        ON activity_genre.activity_id = activity.activity_id
        WHERE LOWER(activity.name) LIKE ?
        AND activity_genre.genre_id = ?
    ...
    not_found = len(activities) == 0  # True if no activities found
    ...
    return render_template("index.html", search=search, activities=displayed_activities, page=page, total_pages=total_pages, tag=tag, not_found=not_found)
